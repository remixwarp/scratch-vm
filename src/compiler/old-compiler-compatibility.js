...
class ConstantInput extends TypedInput {
    constructor (constantValue, safe) {
        super(typeof constantValue === 'string' ? JSON.stringify(constantValue) : `${constantValue}`, TYPE_UNKNOWN);
        this.constantValue = constantValue;
        this.safe = safe;
    }
}

class VariableInput extends TypedInput {
    constructor (source) {
        super(source, TYPE_UNKNOWN);
    }
}

const sanitize = value => JSON.stringify(`${value}`).slice(1, -1);
    ...
    constructor (real) {
        ...
        this.fakeThis = {
            ...
            descendInput (intermediate) {
                if (intermediate.opcode === InputOpcode.CONSTANT) {
                    return new ConstantInput(intermediate.inputs.value, true);
                }
